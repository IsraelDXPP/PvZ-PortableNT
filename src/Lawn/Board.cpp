...
void Board::DrawGameObjects(Graphics* g)
{
...

	{
		for (Plant* aPlant : mPlants)
		{
			...
			if (aPlant->mOnBungeeState == PlantOnBungeeState::NOT_ON_BUNGEE)
			{
				...
				if (mApp->mPlantHealthbars && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN && !aPlantCheck)
				{
					RenderItem& aRenderItem = aRenderList[aRenderItemCount];
	...
	for (int i = 0; i < aRenderItemCount; i++)
	{
		...
		case RenderObjectType::RENDER_ITEM_HEALTHBAR_PLANT:
		{
			...
			Color maxColor = Color(255, 0, 0);
			if (aPlant->mSeedType == SeedType::SEED_IMITATER || aPlant->mImitaterType != SeedType::SEED_NONE)
			{
				...
				for (int i = 0; i <= 2; i++)
				{
					...
					{
					case 0:
						baseColor.mRed = std::min(255, baseColor.mRed + imitaterColor);
						maxColor.mRed = std::min(255, maxColor.mRed + imitaterColor);
						break;
					case 1:
						baseColor.mGreen = std::min(255, baseColor.mGreen + imitaterColor);
						maxColor.mGreen = std::min(255, maxColor.mGreen + imitaterColor);
						break;
					case 2:
						baseColor.mBlue = std::min(255, baseColor.mBlue + imitaterColor);
						maxColor.mBlue = std::min(255, maxColor.mBlue + imitaterColor);
						break;
					}
				}
			}
...
void Board::DrawProgressMeter(Graphics* g)
{
	...
	else if (ProgressMeterHasFlags())
	{
		...
		for (int aFlagWave = 1; aFlagWave <= aNumFlagWaves; aFlagWave++)
		{
			...
			int aPosX = PvzpAnimateCurve(0, mNumWaves, aTotalWavesAtFlag, aFlagsPosEnd, aImagePosX + 6, PvzpCurves::CURVE_LINEAR);
...
void Board::DrawTopRightUI(Graphics* g)
{
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		...
		if (mChallenge->mChallengeState == STATECHALLENGE_ZEN_FADING)
		{
			...
			mStoreButton->mX = PvzpAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 678 + aButtonOffsetX, BOARD_WIDTH, PvzpCurves::CURVE_EASE_IN_OUT);
		}
		else
		{
			mMenuButton->mY = -10;
...
void Board::DrawUIBottom(Graphics* g)
{
	if (mBackground == BackgroundType::BACKGROUND_ZOMBIQUARIUM)
	{
		...
		{
			int aX = i * aWidth + aOffset + (i == aWaves ? aWidth : 0);
			if (i == 0 || i == aWaves)
			{
				PvzpDrawImageCelScaled(
					g, Sexy::IMAGE_WAVESIDE, aX, 40, aWaveTime % Sexy::IMAGE_WAVESIDE->mNumCols,
					aWaveTime / Sexy::IMAGE_WAVESIDE->mNumCols, i == aWaves ? -1.0f : 1.0f, 1.0f
				);
			}
			else
			{
				g->DrawImageCel(Sexy::IMAGE_WAVECENTER, aX, 40, aWaveTime);
			}
		}
...
void Board::DrawFog(Graphics* g)
{
	...
	for (int x = 0; x < MAX_GRID_SIZE_X; x++)
	{
		for (int y = 0; y < MAX_GRID_SIZE_Y + 1; y++)
		{
			...
			float aPosX = x * 80 + mFogOffset - 15 + BOARD_ADDITIONAL_WIDTH;
...
void Board::AnimateBush(int theRow)
{
	Bush* aBush = mBushList[theRow];
	if (aBush == nullptr || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL || !StageHasBushes())
		return;

	aBush->AnimateBush();
	if (!StageHas6Rows() && theRow == 4)
	{
		aBush = mBushList[theRow + 1];
		aBush->AnimateBush();
	}
}
...
int Board::PixelToGridY(int theX, int theY)
{
	...
	int aGridX = PixelToGridX(theX, theY);
	if (aGridX == -1 || theY < LAWN_YMIN + BOARD_OFFSET_Y - 30 || theY > BOARD_HEIGHT - LAWN_YMIN + 50)
		return -1;
	...
	if (StageHasRoof())
	{
		...
		return std::clamp((theY - LAWN_YMIN - BOARD_OFFSET_Y) / 85, 0, MAX_GRID_SIZE_Y - 2);
	}
	else if (StageHasPool())
	{
		return std::clamp((theY - LAWN_YMIN - BOARD_OFFSET_Y) / 85, 0, MAX_GRID_SIZE_Y - 1);
	}
	else
	{
		return std::clamp((theY - LAWN_YMIN - BOARD_OFFSET_Y) / 100, 0, MAX_GRID_SIZE_Y - 2);
	}
}
...
void Board::DrawHealthbar(Graphics* g, Rect rect, Color maxColor, int maxNumber, Color baseColor, int baseNumber, int barWidth, int barHeight, int barOffsetX, int barOffsetY, Color textColor, _Font* textFont, int textOffsetY, Color textOutlineColor, int textOutlineOffset, bool drawBarOutline)
{
	...
	{
		g->SetColor(Color::Black);
		g->DrawRect(Rect(barX - 1, barY - 1, barWidth + 2, barHeight + 2));
	}
